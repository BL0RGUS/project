...
import numpy as np
from tensorflow.keras import backend as K
from tensorflow.keras.layers import Dense, Flatten, Conv2D, AveragePooling2D, ReLU
...
model.add(Conv2D(filters=192, kernel_size=5, strides=1, padding='same'))
model.add(SquareActivation())
model.add(AveragePooling2D(pool_size=(2,2)))

model.add(Conv2D(filters=384, kernel_size=3, strides=1, padding='same'))
model.add(SquareActivation())
