...
int main(int argc, char *argv[]) {
    ...
    if (generate_context > 0) {
        ...
        controller.generate_bootstrapping_and_rotation_keys({1, -1, img_width, -1*img_width,img_size, -1*img_size, (channels*img_size)-num_slots, 2, 4, 8, 14*3, -(784 - 196), (784 - 196) * 10, -2048, num_slots - channels*img_size},
                                                            num_slots,
                                                            true,
 ...
 // try read, encrypt an mnist image into 4096 batch
    ...
    for(int i = 1; i <= 1; i++){
        ...
        Ctxt res = controller.convbn_initial(in, 0.15, false, 28, 1, 5, 3);
        // RELU 1
        res = controller.relu(res, 0.15, false);
        // BOOTSTRAP level 18
        res = controller.bootstrap(res, false);
        // CONV2BN2 (Downsampling)

        vector<Ctxt> res1sx = controller.convbnsx(res, 2, 1, 0.072, false, 28, 1, 5, 3);
        ...
        res = controller.relu(fullpackSx, 0.072, false);
        // FC ONE
        res = controller.fully_connected(res, 1, 1960, 10, 0.0089);

        res = controller.bootstrap(res, false);

        res = controller.relu(res, 0.0089, false);
        // FC TWO
