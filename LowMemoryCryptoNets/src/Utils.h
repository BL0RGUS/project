...
namespace utils {
    ...
    static inline vector<double> read_fc_weight (const string& filename, int features_in, double scale) {
        vector<double> weight = read_values_from_file(filename, scale);
        vector<double> weight_corrected;

        for (int i = 0; i < features_in; i++) {
            weight_corrected.push_back(weight[i]);
        }
