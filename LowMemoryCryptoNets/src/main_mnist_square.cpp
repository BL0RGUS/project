...
int main(int argc, char *argv[]) {
    ...
    if (generate_context > 0) {
        ...
        controller.generate_rotation_keys({1, -1, img_width, -1*img_width,img_size, -1*img_size, (channels*img_size)-num_slots, 2, 4, 8, 14*3, -(784 - 196), (784 - 196) * 10, -2048, num_slots - channels*img_size},
                                                            true,
        ...
        controller.generate_bootstrapping_and_rotation_keys({1, 2048-10},
                                          num_slots/2,
                                          true,
                                          "rotations-fc.bin");
 ...
 // try read, encrypt an mnist image into 4096 batch
    ...
    int labels[50] = {2,0,9,0,3,7,0,3,0,3,5,7,4,0,4,3,3,1,9,0,9,1,1,5,7,4,2,7,4,7,7,5,4,2,6,2,5,5,1,6,7,7,4,9,8,7,8,2,6,7};
    int correct = 0;
    cout << correct << endl;
    for(int i = 1; i <= 1; i++){
        controller.num_slots = 4096;
        num_slots = 4096; 
        input_filename = "../mnist_sample/img_" + to_string(i) + ".jpg";
        cout << input_filename << endl;
        vector<double> input_image = read_image(input_filename.c_str(), 28, 28, 1);

        Ctxt in = controller.encrypt(input_image, 12);
        // CONVBN1

        auto start = start_time();
        controller.load_rotation_keys("rotations-layer1.bin", false);
        Ctxt res = controller.convbn_initial(in, 1.0, false, 28, 1, 5, 3);
        // RELU 1
        res = controller.square(res);
        // BOOTSTRAP level 18
        //res = controller.bootstrap(res, false);
        // CONV2BN2 (Downsampling)

        vector<Ctxt> res1sx = controller.convbnsx(res, 2, 1, 1.0, false, 28, 1, 5, 3);
        ...
        res = controller.square(fullpackSx);
        // FC ONE
        res = controller.fully_connected(res, 1, 1960, 10, 1.0);
        ...
        if (verbose >= 0) {
            cout << "The index of max element is " << index_max << endl;
        }  

        if(index_max == labels[i-1]){
            correct += 1;
        }
    }    
    cout << correct << endl;
