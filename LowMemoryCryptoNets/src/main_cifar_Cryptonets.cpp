...
int main(int argc, char *argv[]) {
    ...
    if (generate_context > 0) {
        ...
        controller.generate_bootstrapping_and_rotation_keys({1, -1, img_width, -1*img_width,img_size, -1*img_size, (channels*img_size)-num_slots, 2, 4, 8, 10*3, -(1024 - 256), (1024 - 256) * 10, -4096, num_slots - channels*img_size},
                                                            num_slots,
                                                            true,
        ...
        controller.generate_bootstrapping_and_rotation_keys({1, 4096-10},
                                          num_slots/2,
                                          true,
                                          "rotations-fc.bin");

        controller.clear_context(num_slots/2);
 ...
 // try read, encrypt an mnist image into 4096 batch
    ...
    for(int i = 0; i < 50; i++){
        ...
        Ctxt res = controller.convbn_initial(in, 0.1057, false, 32, 1, 5, 3);
        ...
        // RELU 1
        res = controller.relu(res, 0.1057, false);
        ...
        vector<Ctxt> res1sx = controller.convbnsx(res, 2, 1, 0.04807, false, 32, 1, 5, 3);

        
        Ctxt fullpackSx = controller.downsample(res1sx[0], res1sx[1], 32, 5, 16, 10);
        res1sx.clear();
        controller.clear_bootstrapping_and_rotation_keys(num_slots);
        //bootstrapping + RELU + bootstrapping
        controller.num_slots = 4096;
        num_slots = 4096; 
        controller.load_bootstrapping_and_rotation_keys("rotations-fc.bin", num_slots, false);
        fullpackSx = controller.bootstrap(fullpackSx, false);

        res = controller.relu(fullpackSx, 0.04807, false);


        //controller.print(res, num_slots, 16, "AFTER relu2");
        //controller.print(res, num_slots, 16, "AFTER CONVBN2");
        auto fcTime = start_time();
        // FC ONE
        res = controller.fully_connected(res, 1, 4096, 10, 0.008887);
        print_duration(fcTime, "FCTime: ");
        res = controller.bootstrap(res, false);

        res = controller.relu(res, 0.008887, false);

        //controller.print(res, 10, 10, "AFTER relu3");

        // FC TWO
        res = controller.fully_connected(res, 2, 10, 10, 1.0);
        print_duration(fcTime, "FCTime: ");
        //controller.print(res, 10, 10, "Decryption: ");
