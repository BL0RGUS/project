...
x_train = np.zeros((len(train_dataloader.dataset), 3, 32, 32))
...
if(training):
    ...
    q_module = compile_torch_model(net, x_train, n_bits=n_bits, rounding_threshold_bits=n_bits, p_error=0.1, device=device)
    ...
    t = time.time()
    for data, target in test_dataloader:
        e = q_module.fhe_circuit.encrypt(q_module.quantize_input(np.array(data)))
        break
    print(f"Encryption: {time.time()-t:.2f}s")
    
    for data, target in test_dataloader:
        e = q_module.fhe_circuit.encrypt(q_module.quantize_input(np.array(data)))
        e = q_module.fhe_circuit.run(e)
        t = time.time()
        d = q_module.dequantize_output(q_module.fhe_circuit.decrypt(e))
        print(f"Decryptiion: {time.time()-t:.2f}s")
        break
